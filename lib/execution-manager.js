...
export default class ExecutionManager {
  ...
  startExecution({runExecArgs} = {}) {
    ...
    this.terminal.writeLn('Running level code …');
  ...
  stopExecution() {
    ...
    if (this.activeDataWriter) {
      this.activeDataWriter.dispose();
    }
  ...
  handleProcessData(data) {
    ...
    this.activeDataWriter = this.writeDataLines(lines);
  }
  ...
  writeDataLines(lines) {
    const interval = setInterval(() => {
      ...
      if (length === 1) {
        ...
        this.writeDataLine(lines.shift());
